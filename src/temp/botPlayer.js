...
const BOT = {
  EVADE_LOOKAHEAD_S:        0.50,   // SECONDS AHEAD TO PREDICT THREAT POSITIONS (HIGHER = PREDICTS THREATS EARLIER)
  LASER_THREAT_RADIUS:      200,     // PX — EVASION BUBBLE AROUND INCOMING LASER (BIGGER BUBBLES = EARLIER EVASION / FEWER HITS)
  GOO_THREAT_RADIUS:        200,    // PX — GOO ARC PREDICTION BUBBLE
  ...
  EVADE_RANGE:              250,    // PX — HOW FAR TO PROJECT THE EVADE TARGET (BIGGER = MORE DECISIVE SIDESTEPS)

  PRISM_VALUE:               90,    // HIGHEST PRIORITY
  TESSERACT_VALUE:           55,
  BOMB_VALUE:                65,
  DIST_PENALTY_PER_PX:       0.35,  // SCORE REDUCTION PER PX OF TRAVEL DISTANCE
  MIN_PICKUP_SCORE:          5,    // IGNORE PICKUPS SCORING BELOW THIS THRESHOLD

  MIN_ENEMY_SCALE_TO_SHOOT:  0.45,  // DON'T SHOOT TINY FAR-AWAY ENEMIES
  MIN_WORM_SCALE_TO_SHOOT:   0.42,

  MOVE_DEADZONE:             26,    // PX — STOP STEERING IF ALREADY THIS CLOSE TO TARGET (SMALLER = MORE PRECISE POSITIONING BUT MORE FIDGITY)
  ...
  STRESS_ENEMY_RADIUS:       220,   // PX — "NEARBY" THRESHOLD (WAS 280 — TIGHTER) (SMALLER = REDUCED ENEMY COUNT STRESS)
  STRESS_ENEMY_CAP:          5,     // ENEMY COUNT THAT MAXES THIS COMPONENT (WAS 3 — WAVE 3 DENSITY NEEDED HIGHER CAP)(HIGHER = CLAMER IN DENSE WAVES)
  STRESS_RISE_RATE:          0.07,  // SLOWER STRESS CLIMB — NOODLE STAYS COMPOSED LONGER (LOWER = COOLER UNDER FIRE)
  STRESS_DECAY_RATE:         0.040, // HOW FAST STRESS FALLS (WAS 0.025 — FASTER RECOVERY AFTER THREATS CLEAR)(HIGHER = FASTER BOUNCE BACK)
  DAMAGE_DECAY_RATE:         26,    // HP/S AT WHICH RECENT-DAMAGE MEMORY FADES (WAS 18 — FADES SOONER)

  // ── IMPERFECTION RANGES ──
  AIM_NOISE_AMP_MIN:         0.01,  // NEAR-PERFECT AIM WHEN CALM
  AIM_NOISE_AMP_MAX:         0.05,  // SLIGHTLY SHAKY AT PEAK STRESS (LOWER = LESS SHAKY WHEN STRESSED)
  AIM_NOISE_SPEED:           1,   // WOBBLE FREQUENCY
  SHOOT_CHANCE_MIN:          0.97,  // FIRES ALMOST EVERY ELIGIBLE FRAME WHEN CALM
  SHOOT_CHANCE_MAX:          0.9,  // MILD HESITATION UNDER STRESS (HIGHER = FEWER MISTAKES UNDER STRESS)
  REACTION_MIN:              0.06,  // FAST TARGET ACQUISITION WHEN CALM (S)
  REACTION_MAX:              0.14,  // SLOWER ACQUISITION UNDER STRESS (LOWER = SNAPPIER TARGET LOCK WHEN STRESSED)
  MOVE_IMPRECISION_MIN:       8,    // PX NAV NOISE WHEN CALM (LOWER FOR CLEANER PATHING)
  MOVE_IMPRECISION_MAX:      28,    // PX NAV NOISE WHEN STRESSED

  // ── BRAIN FART (STRESS) ─────────────────────
  BRAIN_FART_STRESS_MIN:     0.85,  // HIGHER THRESHOLD — ONLY PANICS WHEN TRULY OVERWHELMED (HIGHER = LESS CONFUSION)
  BRAIN_FART_CHANCE_MAX:     0.0015, // LESS FREQUENT CONFUSION
  BRAIN_FART_DURATION_MIN:   0.20,  //  CONFUSION WINDOW
  BRAIN_FART_DURATION_MAX:   0.3,  //
  ...
  // ── SINGULARITY BOMB ──────────────────────────────────────────────────────
  BOMB_STRESS_THRESHOLD:     0.82,  // STRESS THRESHOLD WHEN CALM / FULL LIVES (UPPER BOUND)
  BOMB_STRESS_THRESHOLD_MIN: 0.45,  // THRESHOLD AT MAX DESPERATION — 1 LIFE + LOW HP (LOWER BOUND)
  BOMB_DESPERATION_PANIC:    0.85,  // DESPERATION SCORE THAT BYPASSES STRESS — JUST NEEDS ENEMIES PRESENT
  BOMB_COOLDOWN_S:           8.0,   // SECONDS BETWEEN BOMB USES (PREVENTS RAPID SPAMMING)
};
...
export class BotPlayer {
  ...
  constructor() {
    ...
    // ── SINGULARITY BOMB ─────────────────────────────────────────────
    this._bombCooldown = 0;     // SECONDS UNTIL NEXT BOMB USE IS ALLOWED
  ...
  update(dt, snap) {
    ...
    // ── SINGULARITY BOMB — DESPERATION-SCALED PANIC BUTTON ──────────────
    // DESPERATION BLENDS HP LOSS AND LIVES REMAINING (LIVES WEIGHTED HEAVIER
    // SINCE LOSING THE LAST ONE IS GAME OVER). AT PEAK DESPERATION (LOW HP +
    // 1 LIFE LEFT), NOODLE BYPASSES THE STRESS CHECK AND FIRES AS LONG AS
    // ENEMIES ARE PRESENT. OTHERWISE THE THRESHOLD SLIDES FROM
    // BOMB_STRESS_THRESHOLD DOWN TO BOMB_STRESS_THRESHOLD_MIN AS SHE GETS DESPERATE.
    if (this._bombCooldown > 0) this._bombCooldown -= dt;
    let shouldUseBomb = false;
    if ((ship.bombs ?? 0) > 0 && this._bombCooldown <= 0) {
      const hpFactor    = 1 - (ship.hp / (ship.maxHP || 100));
      const livesFactor = ship.lives <= 1 ? 1.0 : ship.lives <= 2 ? 0.4 : 0.0;
      const desperation = Math.min(1, hpFactor * 0.45 + livesFactor * 0.70);

      const bombThreshold  = _lerp(BOT.BOMB_STRESS_THRESHOLD, BOT.BOMB_STRESS_THRESHOLD_MIN, desperation);
      const enemiesPresent = enemies.some(e => !e.isDead && e.scale >= BOT.MIN_ENEMY_SCALE_TO_SHOOT);
      const panicFire      = desperation >= BOT.BOMB_DESPERATION_PANIC && enemiesPresent;

      if (panicFire || stress >= bombThreshold) {
        shouldUseBomb = true;
        this._bombCooldown = BOT.BOMB_COOLDOWN_S;
      }
    }
    ...
    this._updateOverlay();
    return { aimNX, aimNY, shouldShoot, shouldUseBomb };
  }
